...
def handle_msg(client_socket):
    cmd = client_socket.recv(4).decode()
    ...
    if cmd == "01":
        if user_exists(data_received.split(":")[1]) is True:
            ...
            add_user(data_received.split()[0], data_received.split()[1], data_received.split()[2], md5_encrypt(data_received.split()[3]))
            client_socket.send(create_msg("", "21").encode())
    if cmd == "02":
        if user_exists(data_received.split(":")[1]) is not True:
            client_socket.send(create_msg("", "22").encode())
        elif check_password(username, md5_encrypt(data_received.split(":")[1])) is not True:
                ...
                try:
                    if data == " ":
                        if (time.time() - start_time) > 5 and broadcast_end is False:
                            ...
                            broadcast("Time is up, turn finished@ ending turn", clients)
                            broadcast_end = True
                            client_done = True
                        else:
